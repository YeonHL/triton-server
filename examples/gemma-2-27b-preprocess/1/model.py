        ...
        try:
            for request in requests:
                ...
                if text_input_tensor is not None:
                    text_input_np = text_input_tensor.as_numpy()

                    # Decode the text input if needed
                    text_input = []
                    for outer_array in text_input_np:
                        inner_values = []
                        for inner_item in outer_array:
                            if isinstance(inner_item, bytes):
                                inner_values.append(inner_item.decode("utf-8"))
                            else:
                                inner_values.append(str(inner_item))
                        text_input.append(inner_values)

                    chat_list = []

                    for text in text_input:
                        chat = json.loads(text[0])

                        # Apply chat template
                        result = f"({self.tokenizer.apply_chat_template(
                            chat, tokenize=False
                        )})"

                        chat_list.append(result)

                    # Create output tensor for text_chat
                    text_chat_tensor = pb_utils.Tensor(
                        "text_chat", np.array(chat_list, dtype=np.object_)
                    )
                else:
                    # If no text_input is provided, return empty result
                    text_chat_tensor = pb_utils.Tensor(
                        "text_chat", np.array([""], dtype=np.object_)
                    )

                # Pass through other tensors without modification
                output_tensors = [text_chat_tensor]
